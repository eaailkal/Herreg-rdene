...
require_once( "classes/ManorEntry.class.php" );

$manor_id = isset( $_REQUEST["manor_id"] ) ? (int)$_REQUEST["manor_id"] : 0;

if ( !$member = Manor::getManor( $manor_id ) ) {
...
if ( isset( $_POST["action"] ) and $_POST["action"] == "Save Changes" ) {
  saveManor();
} elseif ( isset( $_POST["action"] ) and $_POST["action"] == "Delete Manor" ) {
  deleteManor();
} else {
...
function saveMember() {
  ...
  $member = new Member( array(
    ...
    "password" => isset( $_POST["password"] ) ? preg_replace( "/[^a-zA-Z0-9]/", "", $_POST["password"] ) : "",
